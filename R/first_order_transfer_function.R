...
E[1L] <- 0
beta[1L] <- 10
y[1L] <- E[1L] + rnorm(n = 1, sd = sd_y)

set.seed(6969)
x <- rep(0, nobs) # rpois(nobs, lambda = 1)
...
all.equal(length(y), length(E), length(beta))
x11()
par(mfrow = c(3, 1))
...
g <- function(theta, x) {
  ...
  lambda_t <- theta[2L]
  beta_t <- theta[3L]
  c(lambda_t * E_t + beta_t * x, lambda_t, beta_t)
}
GG <- function(theta, x) {
  E_t <- theta[1L]
  lambda_t <- theta[2L]
  matrix(data = c(lambda_t, E_t, x,
                  0, 1, 0,
                  0, 0, 1),
         ncol = 3, byrow = TRUE)
}
FF <- matrix(c(1, 0, 0), ncol = 1)

# Matrix of discount factors
discount_factors <- c(0.995, 0.995, 0.995)
...
a0 <- c(0, 0, 0)
R0 <- diag(x = c(100, 0.025, 10), nrow = 3)
...
loglik <- 0
parms_names <- c("E", "lambda", "beta")

data_edu <- read.csv("/home/amenezes/Documents/pesquisa__2020-12-01__modelos_dinamicos/modelagem/2022-08-26__transfer_function/modelos/resultados/exponential_decay_df.csv")

# y <- data_edu[data_edu$mod == "tf_dlm", ]$yt
# x <- data_edu[data_edu$mod == "tf_dlm", ]$xt
# Sequential learning
...
ggplot(data_predictive[data_predictive$t > 6, ], aes(x = t, y = y)) +
  geom_point(size = 2) +
